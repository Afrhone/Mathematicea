...
void setup()
{
  ...
  if (error == 0)
  {
    ...
    gps_status = _4G.gpsStart(arduino4G::GPS_MS_ASSISTED);

    // check answer
    if (gps_status == 0)
    {
      Serial.print(F("2. GPS started in MS-ASSISTED. Time(secs) = "));
...
void loop()
{
  ...
  else
  {
    ...
    if (gps_status == 0)
    {
      Serial.print(F("GPS started in MS-ASSISTED. Time(ms) = "));
      Serial.println(millis() - previous);
    }
    else
    {
      Serial.print(F("Error calling the 'startGPS' function. Code: "));
      Serial.println(gps_status, DEC);
    }
  }
}
