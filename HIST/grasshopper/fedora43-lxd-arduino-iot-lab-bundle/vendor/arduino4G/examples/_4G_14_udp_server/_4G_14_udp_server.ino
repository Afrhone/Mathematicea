...
void loop()
{
  ...
  error = _4G.openSocketListen(socketId, UDP_SERVER, local_port);
  ...
  Serial.println(F("2. Wait for incoming from UDP client (10 secs): "));

  error = _4G.manageSockets(10000);

  if (error == 0)
  {
    if (_4G.socketStatus[socketId].state == arduino4G::STATUS_SUSPENDED_DATA)
    {
      Serial.println(F("\nInfo socket:"));
      Serial.println(F("-------------------------------------"));
    ...
    {
      udp_server_status = false;
      Serial.print(F("3. Incorrect Socket: "));
      Serial.println(_4G.socketStatus[socketId].state, DEC);      
    }
  }
  else
  {
    Serial.print(F("3. No incoming. Code: "));
    Serial.println(error, DEC);
  ...
  while (udp_server_status == true)
  {
    ...
    Serial.print(F("3.2. Waiting to receive data (10 secs): "));

    error = _4G.receive(socketId, 10000);
    ...
    {
      Serial.print(F("No received. Code:"));
      Serial.println(error, DEC);
    }
   ...
   error = _4G.send(socketId, (char *)"This is a message from Cooking Hacks UDP server\n");
    ...
    if (error == 0)
    {
      Serial.println(F("3.3. Sent via UDP socket"));
    }
    else
    {
      Serial.println(F("3.3. Error sending via UDP socket"));
    }
    ...
    if (error == 0)
    {
      ...
      Serial.print(F("3.4. Get socket OK: "));
    ...
    {
      Serial.print(F("3.4. Error getting socket. Error code: "));
      Serial.println(error, DEC);
  ...
  // 4. Close socket
  ///////////////////////////////////////////////////////
  error = _4G.closeSocketListen(socketId, UDP_SERVER);
