...
#include "./utility/arduino4G_constants.h"
...
//! DEFAULT_BOARD
/*! Possible values:
 *  AEDUINO_UNO: due to memory limitations only two sockets available
 *  AEDUINO_MEGA: full six sockets supported.
 *  NOTE: only used if no previous ARDUINO_BOARD declaration.
 */
#define DEFAULT_BOARD      ARDUINO_UNO

#if !defined ARDUINO_BOARD 
  #define ARDUINO_BOARD    DEFAULT_BOARD
#endif
 ...
 *  NOTE: no allowed DEBUG_ARDUINO4G > 0 in Arduino Uno
 */
#define DEBUG_ARDUINO4G     0
...
#define PRINT_LE910(str)    Serial.print(F("[LE910] ")); Serial.print(str);

// APN default settings
#define LE910_GPRS_APN      (char*)"APN"
#define LE910_GPRS_LOGIN    (char*)"user"
#define LE910_GPRS_PASSW    (char*)"password"
...
#define SSL_ACTION_DELETE   0
#define SSL_ACTION_STORE    1
#define SSL_ACTION_READ     2
#define SSL_EMAIL_DISABLED  0
#define SSL_EMAIL_ENABLES   1

// Security data types
#define SSL_TYPE_CERT       0
#define SSL_TYPE_CA_CERT    1
#define SSL_TYPE_RSA        2

// E-mail security
#define EMAIL_NONSSL        0
#define EMAIL_SSL           1

// Define GPS modes
#define LE910_GPS_MS_ASSISTED       0
#define LE910_GPS_MS_BASED          1
#define LE910_GPS_NOT_SUPPORTED     2
#define LE910_GPS_AUTONOMOUS        3

// Patterns for module communication
#define LE910_DATA_TO_MODULE (char*)">>>"
#define LE910_DATA_FROM_MODULE (char*)"<<<"

// Maximum packet size for FTP download must be less than UART _buffer
#define LE910_MAX_DL_PAYLOAD 32

/*!
 * \def UDP_CLIENT
 * \brief Constant for selecting UDP protocol when opening a socket
 */
#define UDP_CLIENT  1

/*! 
 * \def TCP_CLIENT
 * \brief Constant for selecting TCP protocol when opening a socket
 */
#define TCP_CLIENT  0

/*!
 * \def UDP_SERVER
 * \brief Constant for selecting UDP protocol when opening a socket listen
 */
#define UDP_SERVER  1

/*!
 * \def TCP_CLIENT
 * \brief Constant for selecting TCP protocol when opening a socket listen
 */
#define TCP_SERVER  0
...
class arduino4G : public arduinoUART
{
...
private:
    ...
    /*!
    \param  uint8_t method: selected HTTP method:   GET_METHOD
                                                    HEAD_METHOD
                                                    DELETE_METHOD
                                                    POST_METHOD
                                                    PUT_METHOD                                                  
...
public:
    ...
    SocketInfo_t socketInfo[2];
    SocketStatus_t socketStatus[2];
    ...
    */  
    uint8_t openSocket( uint8_t socketId,
                            bool protocol,
                            char* remote_IP,
    ...
    */  
    ...
    \return     0 if OK
                ...
                26 if timeout openning the socket
    */
    uint8_t openSocketListen(uint8_t socketId,
                            bool protocol,
                            uint16_t local_port);
    ...
    */  
    ...
    uint8_t closeSocketListen(uint8_t socketId, uint8_t protocol);
    ...
    */
    ...
    \param uint8_t n:  it is the WDS-Side Stack used by the TA:
        Arduino4G::NETWORK_GSM
        Arduino4G::NETWORK_UTRAN
        Arduino4G::NETWORK_3GPP
        Arduino4G::NETWORK_EUTRAN_ONLY
        Arduino4G::NETWORK_GERAN_UTRAN
        Arduino4G::NETWORK_GERAN_EUTRAN
        Arduino4G::NETWORK_UTRAN_EUTRAN
     */
    ...
    */
    ...
    uint8_t emailSetSenderAddress(char* address);
    uint8_t emailSetSenderUser(char* user);
    uint8_t emailSetSenderPassword(char* password);
    uint8_t emailActivateContext(char* user, char* password);
    uint8_t emailSMTPConfig(uint8_t security, uint16_t port);
    uint8_t emailSend(char* address,char* subject,char* body);
    uint8_t emailResetConfig();
    uint8_t emailSaveConfig();
};

extern arduino4G _4G;
