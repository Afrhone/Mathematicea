...
window.FlowTensorAgent = (()=>{
  ...
  function vector(g){const f=FlowMath.features(g); return [Math.min(1,f.length/3000), Math.min(1,f.curvature/0.05), Math.min(1,f.loops/5), Math.min(1,f.dots/3), g.family==='axiom'?1:0, g.family==='rhythm'?1:0, Math.min(1,(g.functor?.gesture?.momentum||0)), Math.min(1,(g.functor?.gesture?.axiom||0))];}
  async function train(){
    ...
    model=tf.sequential(); model.add(tf.layers.dense({inputShape:[8],units:16,activation:'relu'})); model.add(tf.layers.dense({units:labels.length,activation:'softmax'}));
