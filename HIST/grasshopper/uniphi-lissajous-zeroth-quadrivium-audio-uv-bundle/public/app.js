...
const audioMuteBtn = $('audioMute');
const audioGainEl = $('audioGain');
const audioMixEl = $('audioMix');
const oscATypeEl = $('oscAType');
const oscBTypeEl = $('oscBType');

const exportUVBtn = $('exportUV');
const bakeUVBtn = $('bakeUV');
...
// Audio frequencies:
// - If geometric mode: drive Hz by base * r^n
// - Else: interpret fx/fy multipliers around baseHz (baseHz * multiplier)
const fxHz = (geoMode === 'geo') ? (baseHz * Math.pow(ratio, nx)) : (baseHz * fx);
const fyHz = (geoMode === 'geo') ? (baseHz * Math.pow(ratio, ny)) : (baseHz * fy);
...
// Drive audio (if enabled): frequency from fxHz/fyHz, panning from current point (x,y)
if (audioOn) {
  audio.setFrequencies(params.fxHz, params.fyHz);
  const p = trail.length ? trail[trail.length - 1] : { x: 0, y: 0 };
  audio.setPanFromXY(p.x, p.y, params.amplitude);
  if (!audioMuted) audio.setMasterGain(Number(audioGainEl.value));
}
